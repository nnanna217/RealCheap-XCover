...
function adaptFixture(response, adapt, body) {
  ...
  if (adapt.echoPrice && Array.isArray(r.quotes)) {
    ...
    r.currency = currency;
    if ("total_premium" in r) { r.total_price = total; r.total_price_formatted = fmt(total); r.total_premium = total; r.total_premium_formatted = fmt(total); }
    ...
    if (r.refund) { r.refund.amount = total; r.refund.amount_formatted = fmt(total); r.refund.cancellation_fee_formatted = fmt(r.refund.cancellation_fee || 0); }
  }
  return r;
}
