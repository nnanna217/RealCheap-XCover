...
function applyEvent(body, meta = {}) {
  ...
  if (seen.has(key)) {
    ...
    return { outcome: "duplicate", key, matched_by, order: orders.get(order.transaction_id),
      note: `routed by ${matched_by}; same event already applied (dedup key ignores partner_transaction_id on purpose — the event is the same either way); acknowledged, not re-applied` };
  }
  ...
  if (current && STATUS_RANK[incoming] < STATUS_RANK[current]) {
    ...
    return { outcome: "stale", key, matched_by, order: updated, note: `routed by ${matched_by}; ${event} (${incoming}) arrived after ${current}; ignored` };
  }
  ...
  return { outcome: "applied", key, matched_by, order: updated, note: `routed by ${matched_by}` };
}
