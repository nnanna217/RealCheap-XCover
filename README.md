...
1. **Persist the ledger and make it the unique index it stands in for.** *Evidence, 2026-09-16:* XCover enforces one booking per `partner_transaction_id` for ever; after a server restart the empty in-memory ledger let a browser reuse a reference that had already booked, and XCover refused it — the OMS then held a paid order with a booking it could not see. The `Map` is per-process: a restart loses orders, and two concurrent confirms can both pass the ledger check (XCover's 423/409 is the only thing stopping a double issue today). A single `orders` table with `transaction_id` unique and a conditional update on `booking_id IS NULL` replaces both the Map and the race.
