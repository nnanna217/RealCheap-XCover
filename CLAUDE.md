...
3b. **XCover enforces one booking per `partner_transaction_id`, ever** (live, T16): a new offer confirmed under a reference that already produced a booking is refused with 422 "Duplicated partner_transaction_id" — even after that booking is cancelled. A *failed* confirm does not burn the reference, so retrying under the same reference is safe. Consequences: the ledger must never resurrect a reference it doesn't know (an unknown ref after a restart is minted fresh), and a duplicate-reference 422 is a reconciliation item, not a retry.
...
- **A1 — Eligibility surfaces as a create-offer 422, never a confirm-time one.** *Live (2026-09-16): `E3CCM` on staging has no eligibility rule — it quoted a $4 sleeve at US$1.34. In production that rule lives in the partner's offer schema / catalog classification (configured with the CSE); the prototype keeps XCover as the decider and offers an optional partner-side guard (`RC_ELIGIBLE_CATEGORIES`, off by default) that is recorded as a local decision, never dressed up as an XCover answer.* Fixture uses the documented `offer_quote_generation_failed` (item `offer_quote_pricing_error`, "No rate available for the supplied parameters"). If `E3CCM`'s eligibility is configured as offer-matching rules instead of a rating table, staging may return `offer_not_found_no_match_context` — the checkout treats any create-offer 4xx as "ineligible", so both render the same. The confirm-time `booking_quotes_unsuccessful` is the wrong stage: it would mean a shopper accepted and paid for a plan XCover then couldn't book — that is a fail-open case (refund the premium line, keep the order), not eligibility.
- **A2 — RealCheap collects the premium** as a line item (XCover Single Payment; RealCheap is merchant of record).
- **A3 — SETTLED live (2026-09-16): one quote covers the quantity requested**, and `total_amount` is the rated **total** for that quantity, not a unit price. The prototype no longer multiplies by quantity anywhere; the unit shown is total ÷ quantity, informational only.
...
- **A9 — SETTLED live (2026-09-16): two plans per offer** (`2 Year Plan`, `3 Year Plan`), titled in `content.products[]` (matched by id; `products[].name` is the config slug), benefits in `content.extras`. The checkout renders a plan picker and confirms the chosen quote id. *(Original assumption kept for the record:)* One product per offer. Multiple objects in `products[]` would mean one call returning several plans to choose between — e.g. a 1-year and a 2-year option, or accidental damage vs. extended warranty as separate products. The fixture returns one product because that is the simplest honest case; live, `E3CCM` may return more, and the checkout currently renders `products[0]` only. Confirm with the first live 200; if more come back, the offer column becomes a choice (radio per product, `content.products`-style copy per option) and confirm sends the chosen quote id.
- **A10 — Staging prices are test rates**: randomised per call (the same request twice returned US$191 and US$285), the 3-year plan is often cheaper than the 2-year, and a laptop plan can be ~50% of the laptop price. Shown as returned; never adjusted. Say so in the demo before a panelist does.
- **A11 — `payment_details` is omitted** unless `XCOVER_PAYMENT_PROVIDER` names a real PSP: staging rejects unknown providers (`offer_validation_request_invalid`), and payment here is simulated.
